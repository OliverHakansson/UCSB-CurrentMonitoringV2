...
class VerticalTabsApp(tk.Tk):
    ...
    def _start_experiments_thread(self):
        ...
        # Always save next to the script (like profiles.json/chips.json do),
        # regardless of what the process's current working directory happens
        # to be -- a bare relative filename here was landing wherever the
        # script happened to be launched FROM, not where the script lives.
        self.experiment_file_path = os.path.join(SCRIPT_DIR, f'{profile_name} - {timestamp}.csv')
        with open(self.experiment_file_path, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(headers)
        print(f"[CSV] Saving data to: {self.experiment_file_path}")
